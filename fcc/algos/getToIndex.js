...
function getIndexToIns(arr, num) {
  arr.sort((a, b) => a - b); //sorts the list
  
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] >= num) {
      return i; //returns the index where num is greater than i in arr
    }
  }
  return arr.length; //or return len of arr if no index is found
}
